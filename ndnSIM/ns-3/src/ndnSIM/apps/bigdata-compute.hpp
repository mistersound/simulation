#ifndef NDN_BIGDATA_COMPUTE_H
#define NDN_BIGDATA_COMPUTE_H
namespace ns3 {
namespace ndn {
class Compute;
}
...
public:
	...
	void makeCompute(std::string name) ;
	std::vector<std::string> nameSplit(std::string name);
	void reduceComputation(std::string name);
...
protected:
	...
	std::list<string> adList;
	bool m_firstTime;
	std::list<string> requestList; //we will use a table with status, to keep track of the computation status for the user...
	std::list<string> mapList;
};
